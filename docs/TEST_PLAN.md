...
## 65. Inventory UI — Issue #14

自動Validation:

- HUDにマウスで開ける「かばん」Buttonがある
- 所持品UIはアイコン・名前・個数とお小遣いを表示する
- 開いている間は移動lockする
- 閉じると移動lockを解除し、cancel SFXを鳴らす
- 開いている間の add / money 変更が即時反映される
- Event条件は所持品UIから評価しない
- キュウリアイコンが24x24で存在する
- 既存18 Scene一括Validationが成功する

手動確認:

1. 寝室07:00から開始し、HUDの「かばん」を左クリックして紙面の所持品が開くことを確認する
2. 初期表示が虫取り網 ×1 と お小遣い 0円 であり、クエスト一覧に見えないことを確認する
3. 開いている間は WASD / クリック移動できないことを確認する
4. F3でキュウリをGiveし、開いたまま個数が増えることを確認する
